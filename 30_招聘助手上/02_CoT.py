...
class CandidateDescription(BaseModel):
    ...
    description: str = Field(description="候选人是否匹配的结果")
...
prompt = chat_prompt.format_prompt(jd_str=jd_str,resume_str=resume_str,format_instructions=format_instructions).to_messages()
# 接收用户的询问，返回回答结果
output = llm(prompt)
